...
```{r}
Fatalities
Fatalities$nfrate_18to20 = Fatalities$nfatal1820/Fatalities$pop1820
Fatalities$nfrate_15to17 = Fatalities$nfatal1517/Fatalities$pop1517

Fatalities$frate_18to20 = Fatalities$fatal1820/Fatalities$pop1820
Fatalities$frate_15to17 = Fatalities$fatal1517/Fatalities$pop1517
...
```{r}
Fatalities$jail = relevel(Fatalities$jail, ref="no")
...
mod1_new = lmer(frate~jail+rescale(unemp)+rescale(income)+rescale(beertax)+rescale(youngdrivers)+
                  rescale(miles)+(1|state),data=Fatalities, weights=weight1)
...
anova( lm(frate ~ year + state, data = Fatalities))
```
...
Model 6 (control: drinkage on frate_18to20)
```{r}
mod6 = glm(breath ~ rescale(unemp)+rescale(income)+rescale(beertax)+rescale(youngdrivers)+rescale(miles), family="binomial", data=Fatalities)
prob6 = mod6$fitted.values
pscore6 = ifelse(Fatalities$breath == "yes", prob6,(1-prob6))
weight6 = 1/pscore6
mod6_new = lmer(frate_18to20~drinkage+rescale(unemp)+rescale(income)+rescale(beertax)+rescale(youngdrivers)+rescale(miles)+(1|state)+(1|year),data=Fatalities, weights=weight6)
summary(mod6_new)
```

Model 6 (control: drinkage on frate_15to17)
```{r}
mod6 = glm(breath ~ rescale(unemp)+rescale(income)+rescale(beertax)+rescale(youngdrivers)+rescale(miles), family="binomial", data=Fatalities)
prob6 = mod6$fitted.values
pscore6 = ifelse(Fatalities$breath == "yes", prob6,(1-prob6))
weight6 = 1/pscore6
mod6_new = lmer(frate_15to17~drinkage+rescale(unemp)+rescale(income)+rescale(beertax)+rescale(youngdrivers)+rescale(miles)+(1|state)+(1|year),data=Fatalities, weights=weight6)
summary(mod6_new)
```
